...
// Func?ia de adãugare a unei noi ma?ini în vector
...
// Func?ia de citire a unei masini din fisier
...
// Func?ia de citire a unui vector de masini dintr-un fisier
...
// Func?ia de salvare a unei masini într-un fisier text
void salvareMasinaFisier(FILE* file, Masina masina) {
    fprintf(file, "%d,%d,%.2f,%s,%s,%c\n", masina.id, masina.nrUsi, masina.pret, masina.model, masina.numeSofer, masina.serie);
}

// Func?ia de salvare a unui vector de masini într-un fisier text
void salvareVectorMasiniFisier(const char* numeFisier, Masina* masini, int nrMasini) {
    FILE* file = fopen(numeFisier, "masiniTask.txt");
    if (file == NULL) {
...
// Func?ia de dezalocare a vectorului de masini
